...
workflow <- workflow %>% 
  replace_model(lasso_2)

penalties <- tibble(penalty = 10^seq(-4, -1, length.out = 40))
...
### LASSO Regression
```{r Fit LASSO w Penalty}
glm1_tuned <- logistic_reg(penalty = lr_best$penalty, mixture = 1) %>% # mixture = 1 -> lasso
  set_engine("glmnet")

fit_glm1 <- workflow() %>% 
  add_model(glm1_tuned) %>% 
  add_recipe(glm1_recipe) %>% 
  fit(data = other_data) 
...
fit_glm1 %>% 
  ...
  tidy() %>% 
  filter(estimate > 0) %>%
  dplyr::select(`Term` = term, `Estimate` = estimate) -> lasso_res
```
